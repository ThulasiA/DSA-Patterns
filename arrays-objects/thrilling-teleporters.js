...
function destinations(teleporters, dieSides, startTile, boardSize) {
  const teleporterMap = Object.fromEntries(teleporters.map((pair) => pair.split(',').map(Number)))

  const reachableTiles = new Set()

  for ( let i = 1; i <=dieSides; i++ ) {
    let nextTile = startTile + i;

    if(nextTile > boardSize) {
      continue
    }

    let finalTile = teleporterMap[nextTile] ?? nextTile
    reachableTiles.add(finalTile)
  }
